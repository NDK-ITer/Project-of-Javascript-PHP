...
class EmployerPageState extends State<EmployerPage> {
  ...
  Future<void> getEmployer() async {
    var list =
        await EmployerRepoService().GetAllEmployer(jwt: APIService.JWT());
    ...
    rows.clear();
    setState(() {
      employers = List<Employer>.from(list);
      loading = true;
      // roles = List<Role>.from(rolelist);
      rows.addAll(employers.map((e) => e.toArray()));
    });
  }
